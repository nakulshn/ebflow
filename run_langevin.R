...
parser$add_argument("--T", type = "integer")
...
set.seed(args$seed)
t1 = Sys.time()
print(sprintf("RUN Langevin: %s", res_file_name))
langevin_out = langevin.EB(X, y, sigma, Theta, lambda=args$lambda,
		langevin.inner.iters=args$T, burn.iters=burn.iter,
		eta.phi=eta.phi, tausq.scale=0.5,
		save.iter=save.iter, max.iter=max.iter,
		verbose=TRUE, w.true=w.true)
print("FINISH Langevin")
if (args$predict) {
  print(sprintf("Computing posterior mean"))
  w = langevin_out$w
  phi = langevin_out$hist[[max.iter+1]]$phi
  tmp.out = langevin.EB(X, y, sigma, Theta, lambda=0, langevin.inner.iters=20000,
  		w.init = w, phi.init = phi, eta.phi=eta.phi[max.iter], tausq.scale=0.5,
  		save.iter=50, max.iter=50001, verbose=TRUE, w.true=w.true)
  phi = sapply(seq(1,50001,50), function(it) tmp.out$hist[[it]]$phi)
  theta.mean = posterior.mean(w,phi,Theta,tmp.out$tau)
  langevin_out[["theta.mean"]] = theta.mean
}
t2 = Sys.time()
print(t2-t1)
saveRDS(langevin_out, file=paste0(respath,res_file_name))
